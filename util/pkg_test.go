...
func TestWalkPkgsFunc(t *testing.T) {
	Convey("返回指定目录下满足过滤条件的go package路径列表", t, func() {
		...
		paths, err = WalkPkgsFunc(strings.TrimRight(wd, "util"), IsGoPkg)
		So(err, ShouldBeNil)
		So(paths, ShouldNotBeEmpty)
		So(len(paths), ShouldEqual, 7)
		So(paths, ShouldContain, strings.TrimRight(wd, "util"))
		So(paths, ShouldContain, filepath.Join(strings.TrimRight(wd, "util"), "build"))
		So(paths, ShouldContain, filepath.Join(strings.TrimRight(wd, "util"), "cmd"))
		So(paths, ShouldContain, filepath.Join(strings.TrimRight(wd, "util"), "config"))
		So(paths, ShouldContain, filepath.Join(strings.TrimRight(wd, "util"), "tool"))
		So(paths, ShouldContain, filepath.Join(strings.TrimRight(wd, "util"), "util"))
		So(paths, ShouldContain, filepath.Join(strings.TrimRight(wd, "util"), "variable"))
	...
	Convey("判断是否是golang包目录", t, func() {
		...
		Convey("合法路径", func() {
			...
			Convey("路径下既包含目录，还包含go源文件", func() {
				yes, err := IsGoPkg(strings.TrimRight(wd, "util"))
				So(err, ShouldBeNil)
				So(yes, ShouldBeTrue)
			})
		})
